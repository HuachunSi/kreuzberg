...
mod ffi {
    ...
    extern "Rust" {
        type TableTableGrid;
        #[swift_bridge(init)]
        fn new(columns: Vec<i32>) -> TableTableGrid;
    ...
    extern "Rust" {
        ...
        #[swift_bridge(swift_name = "extractionResult2FromJson")]
        fn extraction_result2_from_json(json: String) -> Result<ExtractionResult2, String>;
        #[swift_bridge(swift_name = "table2FromJson")]
        fn table2_from_json(json: String) -> Result<Table2, String>;
...
pub struct TableTableGrid(pub kreuzberg::extraction::docx::table::TableGrid);
impl TableTableGrid {
    pub fn new(columns: Vec<i32>) -> TableTableGrid {
        let mut __target: kreuzberg::extraction::docx::table::TableGrid = ::std::default::Default::default();
        if let Ok(__v) = ::serde_json::to_value(columns) {
            if let Ok(t) = ::serde_json::from_value(__v) {
                __target.columns = t;
            }
        }
        TableTableGrid(__target)
    }
    pub fn columns(&self) -> Vec<i32> {
        ::serde_json::to_value(&self.0.columns)
            .ok()
            .and_then(|j| ::serde_json::from_value(j).ok())
            .unwrap_or_default()
    }
...
pub struct ExtractionResult2(pub kreuzberg::ocr::ExtractionResult);
...
pub struct Table2(pub kreuzberg::ocr::Table);
impl Table2 {
    pub fn cells(&self) -> String {
        serde_json::to_string(&self.0.cells).expect("serializable cells")
    }
